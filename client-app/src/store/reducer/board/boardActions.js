...
import { deleteCard, deleteColumn, getBoard, getBoardByStage, getCards, moveCard, postCard, postColumn, putCard, putColumn } from "../../../api/board";

export function* loadBoard({ payload }) {
    try {
        let res;
        if (payload.stageId) {
            res = yield getBoardByStage(payload.projectId, payload.stageId);
        } else {
            res = yield getBoard(payload.projectId)
        }
        let board = res.data
